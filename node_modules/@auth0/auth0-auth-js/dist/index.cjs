  ...
  async getTokenByPasskey(options) {
    ...
    try {
      ...
      throw new PasskeyGetTokenError(
        e.error_description || "Failed to exchange passkey credential for tokens.",
        e
      );
    }
  }
...
var AuthClient = class {
  ...
  async getTokenByPassword(options) {
    ...
    if (options.auth0ForwardedFor) {
      ...
      requestConfig[client.customFetch] = ((url, init) => {
  ...
  async verifyLogoutToken(options) {
    ...
    this.#jwks ||= (0, import_jose.createRemoteJWKSet)(new URL(jwksUri), {
      cacheMaxAge: cacheConfig.ttlMs,
      [import_jose.customFetch]: this.#customFetch,
      [import_jose.jwksCache]: this.#jwksCache
    });
